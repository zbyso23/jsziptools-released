...
const core_2 = require("../stream/core");
/**
 * ZipArchiveWriter
 *
 * @example
...
class ZipArchiveWriter {
    ...
    writeDir(path) {
        ...
        if (!this.dirs[path]) {
            this.dirs[path] = true;
            let pathAsBytes = common_1.toBytes(path);
    ...
    writeFile(path, buffer, level) {
        ...
        let dataDescriptor;
        let _crc32;
        let bytes = common_1.toBytes(buffer);
        this.trigger('data', localFileHeader);
        if (level) {
            core_2.deflate({
                buffer: bytes,
...
exports.ZipArchiveWriter = ZipArchiveWriter;
function createLocalFileHeader(fileName, date, isDeflated) {
    ...
    let offset = 0;
    view.setUint32(offset, common_1.LOCAL_FILE_SIGNATURE, true);
...
function createDataDescriptor(crc32, compressedSize, uncompressedSize) {
    ...
    view.setUint32(0, common_1.DATA_DESCRIPTOR_SIGNATURE, true);
...
function createCentralDirHeader(fileName, date, isDeflated, fileOffset, uncompressedSize, compressedSize, crc) {
    ...
    let bytes = new Uint8Array(view.buffer);
    let offset = 0;
    view.setUint32(offset, common_1.CENTRAL_DIR_SIGNATURE, true);
...
function createEndCentDirHeader(numberOfCentralDirs, centralDirHeaderSize, centralDirStartOffset) {
    ...
    view.setUint32(0, common_1.END_SIGNATURE, true); // end of central dir signature
